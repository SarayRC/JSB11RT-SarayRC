...
const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    const [user] = await connection.query(
      `SELECT id, email, type, password FROM user WHERE email = ?`,
      [email]
    );
    ...
    // Creamos un objeto con la info que pasaremos al token, añado type para poder recuperar los datos, en este caso en canEditExperince
    const tokenInfo = {
      id: user[0].id,
      type: user[0].type,
    };
